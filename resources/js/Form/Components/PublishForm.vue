<template>
    <v-sheet class="rounded-lg border mt-4 d-flex flex-column justify-center align-center pa-6">

        <v-card class="mx-auto my-4 w-100" flat>

            <v-form ref="formSettings"  @submit.prevent="handlePublish" >
                <v-card-text class="pa-0">
                    <v-row no-gutters>
                        ...
                        <v-col>
                            ...
                            <div>
                                <QuillEditor v-model:content="afterSubmittingMessage" content-type="html"
                                    toolbar="essential" />
                            </div>
                        </v-col>
                    </v-row>
                    <div class="mt-4 pa-2">
                        ...
                        <div class="d-flex justify-center mt-2">
                            <v-text-field v-model="currentRecipient" variant="outlined" density="compact"
                                ...
                                @keydown="(event: KeyboardEvent) => handleKeydown(event, 'recipient')"
                                ref="recipientInput" :rules="[ requiredRecipientRule, emailRule, sameEmailRule]" class="me-2">
                            </v-text-field>
                    ...
                    <div class="pa-2">
                        ...
                        <div class="d-flex justify-center align-center">
                            <v-text-field v-model="formSlug" label="" variant="outlined" density="compact"
                                :rules="[validBugologUrl, (value: string) => !!value || 'Required']" />
                        </div>
                    </div>
                </v-card-text>
                <v-btn class="mt-6" append-icon="mdi-creation" color="success" type="submit"
                    size="large">
                    Publish
                </v-btn>
            </v-form>
...
const handlePublish = async () => {
    const {valid} = await formSettings.value.validate();
    if (valid) {
        bugFormStore.publishForm();
    }
}
